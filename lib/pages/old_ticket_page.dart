...
class OldTicketPage extends StatefulWidget {
  final Map<String, Future<Box<dynamic>>> boxes;

  const OldTicketPage({super.key, required this.boxes});

  @override
  State<OldTicketPage> createState() => _OldTicketPageState();
}

class _OldTicketPageState extends State<OldTicketPage> {
  bool isLoading = true;
  ...
  void initState() {
    fetchParseTickets(widget.boxes['tickets']!).then((ticketsList) {
      setState(() {
        tickets = ticketsList.where((item) => item.status == 'closed').toList();
